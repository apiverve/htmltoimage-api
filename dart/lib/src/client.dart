...
class HtmltoimageValidationException implements Exception {
  final List<String> errors;

  HtmltoimageValidationException(this.errors);

  @override
  String toString() => 'HtmltoimageValidationException: ${errors.join("; ")}';
}
...
///
/// For more information, visit: https://htmltoimage.apiverve.com?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [html] (required) - HTML code to convert [maxLength: 500000]
/// * [width] - Image width in pixels [min: 100, max: 3840]
/// * [height] - Image height in pixels [min: 100, max: 2160]
...
class HtmltoimageClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'html': ValidationRule(type: 'string', required: true, maxLength: 500000),
    'width': ValidationRule(type: 'integer', required: false, min: 100, max: 3840),
    'height': ValidationRule(type: 'integer', required: false, min: 100, max: 2160),
    'format': ValidationRule(type: 'string', required: false),
  };

  HtmltoimageClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/htmltoimage',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [HtmltoimageValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw HtmltoimageValidationException(errors);
    }
  }

  /// Execute a request to the HTML to Image API.
  ///
  /// Parameters are validated before sending the request.
  Future<HtmltoimageResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw HtmltoimageException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return HtmltoimageResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw HtmltoimageException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw HtmltoimageException('Resource not found');
      } else {
        throw HtmltoimageException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is HtmltoimageException) rethrow;
      throw HtmltoimageException('Request failed: $e');
    }
  }
...
/// Exception thrown by the HTML to Image API client.
class HtmltoimageException implements Exception {
  final String message;

  HtmltoimageException(this.message);

  @override
  String toString() => 'HtmltoimageException: $message';
}
