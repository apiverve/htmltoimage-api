...
class HtmltoimageAPIClientError(Exception):
...
class ValidationError(HtmltoimageAPIClientError):
...
class HtmltoimageAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"html": {"type": "string", "required": True, "maxLength": 500000}, "width": {"type": "integer", "required": False, "min": 100, "max": 3840, "default": 1280}, "height": {"type": "integer", "required": False, "min": 100, "max": 2160, "default": 800}, "format": {"type": "string", "required": False, "default": "png"}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the HtmltoimageAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises HtmltoimageAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/htmltoimage'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise HtmltoimageAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[HtmltoimageAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns HTMLtoImageData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises HtmltoimageAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise HtmltoimageAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise HtmltoimageAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise HtmltoimageAPIClientError(f"Request failed: {str(e)}")
